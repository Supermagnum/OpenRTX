...
def convert_from_uf2(buf):
    ...
    for blockno in range(numblocks):
        ...
        datalen = hd[4]
        if datalen > 476:
            raise ValueError("Invalid UF2 data size at " + str(ptr))
        newaddr = hd[3]
        ...
        if padding < 0:
            raise ValueError("Block out of order at " + str(ptr))
        if padding > 10 * 1024 * 1024:
            raise ValueError("More than 10M of padding needed at " + str(ptr))
        if padding % 4 != 0:
            raise ValueError("Non-word padding size at " + str(ptr))
        while padding > 0:
            padding -= 4
...
def convert_to_uf2(file_content):
    ...
    for blockno in range(numblocks):
        ...
        if len(block) != 512:
            raise ValueError("UF2 block length must be 512")
        outp.append(block)
    return b"".join(outp)
...
def get_drives():
    ...
    if sys.platform == "win32":
        # Fixed argv; used only to list removable drives. nosec B603 B607
