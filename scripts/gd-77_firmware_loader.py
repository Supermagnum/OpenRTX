...
def downloadFirmware(downloadStable):
    ...
    print(
        " - "
        + "Try to download the firmware for your {} from the project page".format(
            outputModes[int(outputFormat)]
        )
    )
    ...
    fwVersionPattern = fwVersionPatternFormat.format(
        "R" if downloadStable == True else "D"
    )
    ...
    if len(urlFW):
        global downloadedFW
        downloadedFW = os.path.join(
            tempfile.gettempdir(), next(tempfile._get_candidate_names()) + ".sgl"
        )

        print(
            " - " + "Downloading the firmware version {}, please wait".format(fwVersion)
        )
...
def sendFileData(fileBuf, dev):
    ...
    while address < fileLength:
        ...
        dataHeader = updateBlockAddressAndLength(
            dataHeader, address, DATA_TRANSFER_SIZE
        )
            ...
            if (address % 0x400) == 0:
                ...
                if (
                    sendAndCheckResponse(
                        dev,
                        createChecksumData(fileBuf, checksumStartAddress, address),
                        responseOK,
                    )
                    == False
                ):
...
def probeModel(dev):
    ...
    command0 = [
        [0x44, 0x4F, 0x57, 0x4E, 0x4C, 0x4F, 0x41, 0x44],
        [0x23, 0x55, 0x50, 0x44, 0x41, 0x54, 0x45, 0x3F],
    ]  # 'DOWNLOAD'
    ...
    models = [
        ["DV01", SGLFormatOutput.GD_77],
        ["DV02", SGLFormatOutput.GD_77S],
        ["DV03", SGLFormatOutput.DM_1801],
    ]
    ...
    while commandNumber < len(commandID):
        if (
            sendAndCheckResponse(
                dev, commandID[commandNumber][0], commandID[commandNumber][1]
            )
            == False
        ):
...
def sendInitialCommands(dev, encodeKey):
    ...
    command1 = [commandLetterA, responseOK]
    command3 = [
        [0x46, 0x2D, 0x50, 0x52, 0x4F, 0x47, 0xFF, 0xFF],
        responseOK,
    ]  # ... F-PROG..
    ...
    if outputFormat == SGLFormatOutput.GD_77:
        command2 = [
            ...
            [0x44, 0x56, 0x30, 0x31],
        ]  # .... DV01enhi (DV01enhi comes from deobfuscated sgl file)
        command4 = [
            [
                0x53,
                0x47,
        ...
        command5 = [
            [0x4D, 0x44, 0x2D, 0x37, 0x36, 0x30, 0xFF, 0xFF],
            responseOK,
        ]  # MD-760..
    elif outputFormat == SGLFormatOutput.GD_77S:
        command2 = [
            [0x44, 0x56, 0x30, 0x32, 0x6D, 0x40, 0x7D, 0x63],
            [0x44, 0x56, 0x30, 0x32],
        ]  # .... DV02Gpmj (thanks Wireshark)
        command4 = [
            [
                0x53,
                0x47,
        ...
        command5 = [
            [0x4D, 0x44, 0x2D, 0x37, 0x33, 0x30, 0xFF, 0xFF],
            responseOK,
        ]  # MD-730..
    elif outputFormat == SGLFormatOutput.DM_1801:
        command2 = [
            [0x44, 0x56, 0x30, 0x33, 0x74, 0x21, 0x44, 0x39],
            [0x44, 0x56, 0x30, 0x33],
        ]  # .... last 4 bytes of the command are the offset encoded as letters a - p (hard coded fr
        command4 = [
            [
                0x42,
                0x46,
        ...
        command5 = [
            [0x31, 0x38, 0x30, 0x31, 0xFF, 0xFF, 0xFF, 0xFF],
            responseOK,
        ]  # MD-1801
    elif outputFormat == SGLFormatOutput.RD_5R:
        command2 = [
            [0x44, 0x56, 0x30, 0x32, 0x53, 0x36, 0x37, 0x62],
            [0x44, 0x56, 0x30, 0x32],
        ]  # .... last 4 bytes of the command are the offset encoded as letters a - p (hard coded fr
        command4 = [
            [
        ...
        command5 = [
            [0x42, 0x46, 0x2D, 0x35, 0x52, 0xFF, 0xFF, 0xFF],
            responseOK,
        ]  # BF-5R

    command6 = [
        [0x56, 0x31, 0x2E, 0x30, 0x30, 0x2E, 0x30, 0x31],
        responseOK,
    ]  # V1.00.01
    commandErase = [
        [0x46, 0x2D, 0x45, 0x52, 0x41, 0x53, 0x45, 0xFF],
        responseOK,
    ]  # F-ERASE
    commandPostErase = [commandLetterA, responseOK]
    commandProgram = [
        [0x50, 0x52, 0x4F, 0x47, 0x52, 0x41, 0x4D, 0xF],
        responseOK,
    ]  # PROGRAM
    commands = [
        command0,
    ...
    while commandNumber < len(commands):
        ...
        if (
            sendAndCheckResponse(
                dev, commands[commandNumber][0], commands[commandNumber][1]
            )
            == False
        ):
...
def usage():
    ...
    print(
        "    -m, --model=<type>             : Select transceiver model. Models are: {}".format(
            ", ".join(str(x) for x in outputModes[:-1])
        )
        + "."
    )
    print(
    ...
    print(
        "    -S, --stable                   : Select the stable version while downloading from the project page"
    )
    print(
        "    -U, --unstable                 : Select the development version while downloading from the project page"
    )
    print("")
...
def main():
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:],
            "hf:m:dSUF",
            ["help", "firmware=", "model=", "download", "stable", "unstable", "force"],
        )
    ...
    if dev:
        ...
        print(
            " - "
            + 'Now flashing your {} with "{}"'.format(
                outputModes[int(outputFormat)], sglFile
            )
        )
        ...
        if outputFormat == SGLFormatOutput.GD_77:
            ...
            encodeKey = [
                ...
                (0x7D),
                (0x63),
            ]  ## Original header (smaller filelength): was (0x47), (0x70), (0x6d), (0x4a)
        elif outputFormat == SGLFormatOutput.DM_1801:
        ...
        if file_extension == ".sgl":
            firmwareModelTag = {
                SGLFormatOutput.GD_77: 0x1B,
                SGLFormatOutput.GD_77S: 0x70,
                SGLFormatOutput.DM_1801: 0x4F,
                SGLFormatOutput.RD_5R: 0x5C,
            }
        ...
        if sendInitialCommands(dev, encodeKey) == True:
            if sendFileData(fileBuf, dev) == True:
                print(
                    "Firmware update complete. Please reboot the {}.".format(
                        outputModes[int(outputFormat)]
                    )
                )
            else:
